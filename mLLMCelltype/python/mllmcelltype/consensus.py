...
from .providers.openrouter import get_openrouter_balance
...
def _summarize_usage(usage: dict[str, Any] | None) -> tuple[float | int | None, float | None]:
    """Extract token and credit-like cost from provider usage payload."""
    if not usage:
        return None, None
    ...
    credits = usage.get("total_cost") or usage.get("cost") or usage.get("total_credits")
...
def interactive_consensus_annotation(
    ...
    Args:
        ...
        track_usage: If True, collect usage metadata per model (when available) and
            aggregate token/credit information.
    ...
    # Check OpenRouter balance before starting if track_usage is enabled
    initial_balance = None
    final_balance = None
    actual_credits_used = None
    
    if track_usage and "openrouter" in api_keys:
        initial_balance = get_openrouter_balance(api_keys["openrouter"])
        if initial_balance is not None:
            write_log(f"Initial OpenRouter balance: ${initial_balance:.6f}")
    ...
    for model_item in models:
        ...
        try:
            ...
            annotations_only = results
            usage_meta: dict[str, Any] | None = None

            if track_usage and isinstance(results, tuple) and len(results) == 2:
                annotations_only, meta = results
            ...
            model_results[model_name] = annotations_only

            if track_usage:
                tokens_used, credits_used = _summarize_usage(usage_meta)
    ...
    # Check OpenRouter balance after completion if track_usage is enabled
    if track_usage and "openrouter" in api_keys:
        final_balance = get_openrouter_balance(api_keys["openrouter"])
        if final_balance is not None and initial_balance is not None:
            actual_credits_used = initial_balance - final_balance
            write_log(f"Final OpenRouter balance: ${final_balance:.6f}")
            write_log(f"Actual credits used (balance difference): ${actual_credits_used:.6f}")
            
            # If we have accumulated credits from usage info, compare them
            if credits_seen and total_credits > 0:
                write_log(f"Accumulated from API responses: ${total_credits:.6f}")
                difference = abs(actual_credits_used - total_credits)
                if difference > 0.01:  # More than 1 cent difference
                    write_log(
                        f"Warning: Balance difference (${actual_credits_used:.6f}) differs from accumulated "
                        f"usage (${total_credits:.6f}) by ${difference:.6f}",
                        level="warning"
                    )
    ...
    usage_summary = {
        ...
        "total_credits": total_credits if credits_seen else None,
        "initial_balance": initial_balance,
        "final_balance": final_balance,
        "actual_credits_used": actual_credits_used,
    }
