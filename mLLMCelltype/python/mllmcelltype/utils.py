...
def format_results(results: list[str] | dict[str, Any] | str, clusters: list[str]) -> dict[str, str]:
    """Format results into a dictionary mapping cluster names to annotations.

    Args:
        results: Annotation results (list of lines, single string, or dict with a
                 ``lines``/``content``/``results``/``data`` payload)
    ...
    if isinstance(results, dict):
        for key in ("lines", "content", "results", "data"):
            if key in results and results[key] is not None:
                results = results[key]
                break

    if isinstance(results, str):
        normalized_results = results.splitlines()
    elif isinstance(results, list):
        normalized_results = results
    else:
        normalized_results = [str(results)] if results is not None else []

    # Clean up results (remove empty lines and whitespace)
    clean_results: list[str] = []
    for line in normalized_results:
        if line is None:
            continue
        text = str(line).strip()
        if text:
